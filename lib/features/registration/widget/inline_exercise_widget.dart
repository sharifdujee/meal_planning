...
import '../../week/provider/workout_provider.dart';
...
class InlineExerciseWidget extends ConsumerWidget{
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AnimatedSwitcher(
        ...
        child: workoutExercise.isEditing ? _buildEditForm(context,ref)
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFF2D3A35), borderRadius: BorderRadius.circular(8.r)),
            child: CustomText(text: "${workoutExercise.sets.length}", color: const Color(0xFF469271), fontWeight: FontWeight.bold),
          ),
          SizedBox(width: 12.w),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              CustomText(text: workoutExercise.name.isEmpty ? "Nuevo Ejercicio" : workoutExercise.name, color: Colors.white, fontWeight: FontWeight.w600),
              CustomText(text: "${workoutExercise.sets.length} Serie • ${workoutExercise.restTimeInMinutes} min", color: Colors.grey, fontSize: 12.sp),
            ],
          ),
          const Spacer(),
          IconButton(
  ...
  Widget _buildEditForm(BuildContext context, WidgetRef ref) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          CustomText(text: "Editar ejercicio", fontSize: 18.sp, fontWeight: FontWeight.bold, color: Colors.white),
          SizedBox(height: 16.h),

          // Reusable Input Fields (Nombre, Descanso, etc.)
          _buildFieldLabel("Nombre del ejercicio"),
          CustomText(
            text: "Ej: Press de banca",
          ...
          // Sets Section
          const CustomText(text: "Series", color: Colors.white, fontWeight: FontWeight.w600),
          ...
          Row(
            children: [
              Expanded(
                  child: _actionButton(
                      text: "Volver",
                      onTap: () => null, //notifire.toggleEdit(workoutExercise.id),
                      isOutlined: true
                  )
              ),
              SizedBox(width: 12.w),
